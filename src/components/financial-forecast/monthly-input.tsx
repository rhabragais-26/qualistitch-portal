...
export function MonthlyForecastInput() {
  ...
  useEffect(() => {
    if (editingRecord) {
      ...
      form.reset({ month: format(new Date(), 'yyyy-MM'), categoryId: '', department: '', amount: 0, notes: '' });
    }
  }, [editingRecord, form]);
  ...
  const onSubmit = async (values: ForecastFormValues) => {
    ...
    const docId = editingRecord?.id || uuidv4();
    ...
    try {
      ...
      form.reset({ month: values.month, categoryId: '', department: '', amount: 0, notes: '' });
  ...
  }, [forecasts, monthFilter, categoryFilter, searchTerm, categories, departmentFilter]);
                        ...
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                            <fieldset disabled={!canEdit} className="space-y-6">
                                ...
                                <div className="flex justify-end gap-2">
                                    ...
                                    <Button type="button" variant="ghost" onClick={() => form.reset({ month: form.getValues('month'), categoryId: '', department: '', amount: 0, notes: '' })}>Clear</Button>
                ...
                <CardContent>
                    <div className="border rounded-md h-[60vh] overflow-y-auto modern-scrollbar">
                        <Table>
                            ...
                            <TableBody>
                                {isLoading ? [...Array(5)].map((_, i) => <TableRow key={i}><TableCell colSpan={canEdit ? 7 : 6}><Skeleton className="h-8 w-full"/></TableCell></TableRow>)
                                    : error ? <TableRow><TableCell colSpan={canEdit ? 7 : 6} className="text-center text-destructive">Error: {error.message}</TableCell></TableRow>
                                    ...
                                    )) : <TableRow><TableCell colSpan={canEdit ? 7 : 6} className="text-center text-muted-foreground">No records found.</TableCell></TableRow>}
                            </TableBody>
                        </Table>
                    </div>
                </CardContent>
